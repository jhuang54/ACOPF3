function [groups, isolated, islandStatus] = find_islands(mpc)
...
%   MATPOWER
%   $Id: find_islands.m,v 1.1 2012/01/31 20:49:26 cvs Exp $
...
if nnz(C)
    ...
    %Remaining nodes are all isolated, hence no islanding can happen beyond
    %this level
    isolated = mpc.bus( mpc.bus(:,2) ~= 4 , 1 );
    %No need to extract further islands
    islandStatus = -1;
    groups = [];
end