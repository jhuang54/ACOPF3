...
class result:
    def __init__(self,n_snaps,ngen,nshunt):
        ...
        self.dTQs=np.zeros(n_snaps)
        self.rdTQs=np.zeros(n_snaps)
...
for id_t in range(0,n_snaps):
    ...
    for iter in range(iter_max):
        ...
        # dTPg=abs(pg_t_m-pAg_t)
        # dvg=abs(vg_t_m-vg_t)
        
        # if dpld.max()>1e-2 or dqld.max()>1e-2 or dTPg.max()>1e-3 or dvg.max()>2e-3:
        #     print('iteration:%d: large mismatch' %iter)
            
        # print('maximum mismatch:')
        # print('pld:%.8f' %dpld.max())
        # print('qld:%.8f' %dqld.max())
        # print('pg:%.8f' %dTPg.max())
        # print('v:%.8f' %dvg.max())
    ...
    result1.dTQs[id_t]=sum(abs(result1.dQs[:,id_t]))      
    result1.rdTQs[id_t]=result1.dTQs[id_t]/sum(qshunt_max)
    ...
    result1.dTsteps_Qs[id_t]=result1.dTQs[id_t]/Bi
...
# ax1.plot(range(1,n_snaps+1),result1.dTQg*sbase,'.',markersize=2.5,color=color)
# ax1.tick_params(axis='y', labelcolor='tab:red')
# ax1.set_yticks([0,0.0005,0.001,0.0015])
# #plt.ylim(Padja_ylim)
# ax2=ax1.twinx()
# color='tab:green'
# ax2.set_ylabel('Percent %', color=color)  # we already handled the x-label with ax1
# ax2.plot(range(1,n_snaps+1),result1.rdTQg*100,'*',markersize=2.5, color=color)
# ax2.tick_params(axis='y', labelcolor=color)
# plt.xlim([1,n_snaps+1])
# #plt.ylim(Padjr_ylim)
...
ax1.set_xticklabels(xlabels, rotation=45)
ax1.set_ylabel('Mvar',color=color)
ax1.plot(range(1,n_snaps+1),result1.dTQs*sbase,'.',markersize=2.5,color=color)
ax1.tick_params(axis='y', labelcolor='tab:red')
ax1.set_xlabel('Time')
#ax1.set_yticks([0,0.0005,0.001,0.0015])
#plt.ylim(Padja_ylim)
...
color='tab:green'
ax2.set_ylabel('Percent %', color=color)  # we already handled the x-label with ax1
ax2.plot(range(1,n_snaps+1),result1.rdTQs*100,'*',markersize=2.5, color=color)
ax2.tick_params(axis='y', labelcolor=color)
plt.xlim([0,n_snaps+1])
#plt.ylim(Padjr_ylim)
...
ax.plot(range(1,n_snaps+1),result1.dTsteps_Qs,'.',markersize=2.5,color=color)
...
ax.plot(range(1,n_snaps+1),result1.adTsteps_Qs,'.',markersize=2.5,color=color)
ax.tick_params(axis='y', labelcolor='tab:red')
#ax1.set_yticks([0,0.0005,0.001,0.0015])
#plt.ylim(Padja_ylim)
plt.xlim([0,n_snaps+1])
