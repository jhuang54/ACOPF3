...
icase = 'Maui2022dm_rd_v33_SwitchShuntsNoPhaseshift'
...
# # Ybus
# Ybus_t = net_t._ppc['internal']['Ybus']

# # # power flow solution
# vm = net_t.res_bus.vm_pu.values
# va_dg = net_t.res_bus.va_degree.values
# va = np.exp(1j * va_dg * np.pi / 180)
# v = vm * va

# vm0=net_t.ext_grid.vm_pu[0]
# va0=np.exp(1j*net_t.ext_grid.va_degree[0]*np.pi/180)
# v0=vm0*va0

# # check Ybus is correct or not by calculating power injection with Ybus
# mappd2ppc_t = net_t._pd2ppc_lookups["bus"]
# Ybus_t = Ybus_t[mappd2ppc, 0:nbus]
# Ybus_t = Ybus_t[0:nbus, mappd2ppc]
# # I_t = Ybus.dot(v)
# # S_t = v * np.conj(I)
# dYbus=Ybus-Ybus_t
...
for iter in range(iter_max):
    ...
    # vm_net=net.res_bus.vm_pu.to_numpy()
    # vmll_net=np.delete(vm_net, busid_slack)
    # dvmll=abs(abs(vll)-vmll_net)
    # print('dvmll:',max(dvmll))
    
    # va_net=net.res_bus.va_degree.to_numpy()
    # vall_net=np.delete(va_net, busid_slack)
    # vall=np.angle(vll)*180/np.pi
    # dvall=abs(vall-vall_net)
    ...
    # if dpld.max()>1e-3 or dqld.max()>1e-3 or dpg.max()>1e-3 or dvg.max()>1e-3 or dpsg.max()>1e-3 or dqsg.max()>1e-3:
    #     print('iteration:%d: large mismatch' %iter)
        
    # vm_net_t=net_t.res_bus.vm_pu.to_numpy()
    # vmll_net_t=np.delete(vm_net_t, busid_slack)
    # dvmll=abs(abs(vll)-vmll_net_t)
    # print('dvmll:',max(dvmll))
    
    # va_net_t=net_t.res_bus.va_degree.to_numpy()
