...
if ACorDCOPF=='0':
    ...
    if ControlSwitchS=='0':
       ...
       epsi_qshunt=0.05 
epsi_l=0.05# dual variables associating with voltage constraints
...
class result:
    def __init__(self,n_snaps):
        ...
        self.dPg=np.zeros(n_snaps)
        self.rdPg=np.zeros(n_snaps)
        self.dQg=np.zeros(n_snaps)
        self.rdQg=np.zeros(n_snaps)
        self.dQs=np.zeros(n_snaps)
        self.rdQs=np.zeros(n_snaps)
        self.dsteps_Qs=np.zeros(n_snaps)
        self.adsteps_Qs=np.zeros(n_snaps)
...
result0=result(n_snaps)
result1=result(n_snaps)
...
#for id_t in range(1):
    ...
    result1.dPg[id_t]=sum(abs(pg_t-pg))
    if sum(abs(pg))>0:
        result1.rdPg[id_t]=result1.dPg[id_t]/sum(abs(pg))
    result1.dQg[id_t]=sum(abs(qg_t-qg))
    if sum(abs(qg))>0:
        result1.rdQg[id_t]=result1.dQg[id_t]/sum(abs(qg))
    result1.dQs[id_t]=sum(abs(qshunt_t-qshunt))      
    result1.rdQs[id_t]=result1.dQs[id_t]/sum(qshunt_max)
    result1.dsteps_Qs[id_t]=result1.dQs[id_t]/Bi
    result1.adsteps_Qs[id_t]=sum(result1.dsteps_Qs)
    qshunt=qshunt_t
# # #path_plt = os.path.join(path_cur, 'Plot')
# iplt=1      
# plt.figure(iplt) 
...
iplt+=1
plt.figure(iplt)    
plot_vmax_DCOPF=plt.plot(range(1,n_snaps+1), result0.vmax,'*',markersize=3,color='red')
plot_vmin_DCOPF=plt.plot(range(1,n_snaps+1), result0.vmin,'*',markersize=3,color='blue')
plot_vmax_EDCOPF=plt.plot(range(1,n_snaps+1), result1.vmax,'.',markersize=3,color='green')
plot_vmin_EDCOPF=plt.plot(range(1,n_snaps+1), result1.vmin,'.',markersize=3,color='orange')
plot_upper=plt.plot(range(1,n_snaps+1), v_u*np.ones(n_snaps),linestyle='dashed',color='black')
plot_lower=plt.plot(range(1,n_snaps+1), v_l*np.ones(n_snaps),linestyle='dashed',color='black')
plt.legend((plot_vmax_DCOPF[0], plot_vmin_DCOPF[0],plot_vmax_EDCOPF[0], plot_vmin_EDCOPF[0],plot_upper[0],plot_lower[0]), ('DCOPF Max V', 'DCOPF Min V','EDCOPF Max V', 'EDCOPF Min V','Upper bound','Lower bound'))
plt.xlabel('Time')
plt.ylabel('Voltage magnitude, p.u.')
plt.ylim([0.7,1.15])
plt.title('Voltage')
plt.grid(True)
plt.savefig(path_plt+'/'+OPF_type+'/v.png', dpi=400)    
...
# ax1.plot(range(1,n_snaps+1),result1.dPg*sbase,'.',markersize=2.5,color=color)
# ax1.tick_params(axis='y', labelcolor='tab:red')
# ax1.set_yticks([0,0.0005,0.001,0.0015])
# #plt.ylim(Padja_ylim)
# ax2=ax1.twinx()
...
# ax2.plot(range(1,n_snaps+1),result1.rdPg*100,'*',markersize=2.5, color=color)
# ax2.tick_params(axis='y', labelcolor=color)
# plt.xlim([1,n_snaps+1])
# #plt.ylim(Padjr_ylim)
...
# ax1.plot(range(1,n_snaps+1),result1.dQg*sbase,'.',markersize=2.5,color=color)
# ax1.tick_params(axis='y', labelcolor='tab:red')
# ax1.set_yticks([0,0.0005,0.001,0.0015])
# #plt.ylim(Padja_ylim)
# ax2=ax1.twinx()
# color='tab:green'
# ax2.set_ylabel('Percent %', color=color)  # we already handled the x-label with ax1
# ax2.plot(range(1,n_snaps+1),result1.rdQg*100,'*',markersize=2.5, color=color)
# ax2.tick_params(axis='y', labelcolor=color)
# plt.xlim([1,n_snaps+1])
# #plt.ylim(Padjr_ylim)
...
ax1.set_ylabel('Mvar',color=color)
ax1.plot(range(1,n_snaps+1),result1.dQs*sbase,'.',markersize=2.5,color=color)
ax1.tick_params(axis='y', labelcolor='tab:red')
#ax1.set_yticks([0,0.0005,0.001,0.0015])
#plt.ylim(Padja_ylim)
ax2=ax1.twinx()
color='tab:green'
ax2.set_ylabel('Percent %', color=color)  # we already handled the x-label with ax1
ax2.plot(range(1,n_snaps+1),result1.rdQs*100,'*',markersize=2.5, color=color)
ax2.tick_params(axis='y', labelcolor=color)
plt.xlim([1,n_snaps+1])
#plt.ylim(Padjr_ylim)
plt.title('Shunt Reactive Power Adjustment')
plt.grid(True)
plt.savefig(path_plt+'/'+OPF_type+'/Qshunt_Adjust.png', dpi=400)  
...
color='tab:red'
ax.set_xlabel('Time')
ax.set_ylabel('Change of steps',color=color)
ax.plot(range(1,n_snaps+1),result1.dsteps_Qs,'.',markersize=2.5,color=color)
ax.tick_params(axis='y', labelcolor='tab:red')
#ax1.set_yticks([0,0.0005,0.001,0.0015])
#plt.ylim(Padja_ylim)
plt.xlim([1,n_snaps+1])
#plt.ylim(Padjr_ylim)
plt.title('The number of switch shunt step changes')
plt.grid(True)
...
ax.set_xlabel('Time')
ax.set_ylabel('Change of steps',color=color)
ax.plot(range(1,n_snaps+1),result1.adsteps_Qs,'.',markersize=2.5,color=color)
ax.tick_params(axis='y', labelcolor='tab:red')
#ax1.set_yticks([0,0.0005,0.001,0.0015])
#plt.ylim(Padja_ylim)
plt.xlim([1,n_snaps+1])
